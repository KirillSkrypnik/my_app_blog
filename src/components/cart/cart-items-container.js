...
const CartItemsContainer = () => {
	...
	// функция обработки чистой корзины
	...
	// Если корзина заполнена вернется это
	return (
		<div className="content_wrap_cart">
			{ cart ? (
				<div className="content_wrap_cart_flex">
					{/*Cart Items*/ }
					<div className="content_wrap_cart_table_item">
						{ cartItems.length &&
						cartItems.map( ( item ) => (
							<CartItem
								key={ item.product_id }
					...
					{/*Cart Total*/ }
					<div className="content_wrap_cart_table_total">
						<h2>Cart Total</h2>
